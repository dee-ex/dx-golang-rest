...
func LoginHandler(w http.ResponseWriter, r *http.Request) {
    ...
    login_message, err := serv.CreateNewToken(user)
    if err != nil {
        http.Error(w, err.Error(), 500)
        return
    }
...
func ProfileHandler(w http.ResponseWriter, r *http.Request) {
    ...
    profile_message.ID = int(r.Context().Value("id").(float64))
    profile_message.Username = r.Context().Value("username").(string)

    responses.JSONResponse(w, 200, &profile_message)
}
